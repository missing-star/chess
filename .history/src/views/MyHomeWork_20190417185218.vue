    ...
    getChessDetail(id) {
      this.$axios({
          ...
          if(res.data.status == 1) {
              ...
              JSON.parse(res.data.data.task.chess_maneual.data_code).forEach(array => {
                  let temp = [];
                  array.forEach(item => {
                      temp.push(item)
                  });
              ...
              initChess();
              JSON.parse(res.data.data.task.chess_maneual.data_text).forEach(item => {
                  this.recordList.push(item);
              });
              this.answerList = res.data.data.task.chess_maneual.play_log;
          }
      }).catch((err) => {
          console.log(err);
      });
