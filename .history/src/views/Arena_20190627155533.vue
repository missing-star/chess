<template>
  <div class="chess-arena-wrapper">
    <!-- 背景音乐 -->
    <audio ref="arenaBg" preload="auto" muted loop>
      <source src="../assets/audio/竞技场背景音乐.mp3">
    </audio>
    ...
    <chess-check-point-panel :level="selectedLevel" :begin="beginNum" :end="endNum" :show-title="selectedTitle"
      @hide="hideCheckPointPanel" :is-show="showCheckPointPanel"></chess-check-point-panel>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goGame(flag) {
        ...
        if (this.isOnline) {
          ...
          this.goOnlineRace();
          if (this.socket == null) {
            this.socket = new WebSocket("ws://154.92.18.110:1234");
          }
          ...
          this.socket.onmessage = msg => {
            if (msg.data.indexOf("b login success") != -1) {
              ...
              if (!sessionStorage.getItem("user_type2")) {
                ...
                // 将自己的用户ID,uuid发送给黑方，用来获取用户昵称，头像信息
                this.socket.send(
                  `${sessionStorage.getItem("uuid")}-${sessionStorage.getItem(
                  "user_type"
                )}-${JSON.stringify({
          ...
          this.waitTime = 0;
          gameSocket.close();
          var machineCode = `user${this.getUuuid(8, 16)}`;
          //发送请求确定该对战为人机
          this.$axios({
            ...
            data: this.qs.stringify({
              ...
              b_user_socket_key: machineCode
            })
          });
          //人机
          sessionStorage.setItem("user_type", "a");
          sessionStorage.setItem("uuid", this.uuid);
          sessionStorage.setItem("code", machineCode);
          this.isOnline = false;
          sessionStorage.setItem("isRed", true);
          sessionStorage.setItem("nowWho", 0);
    ...
    created() {
      this.$nextTick(() => {
        this.$refs.arenaBg.addEventListener('canplay', () => {
          if (localStorage.getItem('isCloseBg') == 'false') {
            this.$refs.arenaBg.play();
          }
        });
      });
    },
