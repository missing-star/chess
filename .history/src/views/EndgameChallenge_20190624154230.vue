<template>
  <div class="chess-my-homework-wrapper">
    <!-- 背景音乐 -->
    <audio ref="endBg" preload="auto" muted loop>
      <source src="../assets/audio/象棋闯关.mp3">
    </audio>
    ...
    <!-- 失败提示框 -->
    <lose-game :is-show="showLostAlert" :ImgShow="ImgShow" :avter="avter" @do-again="getCheckPointDetail"></lose-game>

    <!-- 成功提示框 -->
    <water-box :is-show="showWaterBox" :waterImg="waterImg" @hide="hideWaterBox" :againImg="againImg" :nextImg="nextImg"
      @do-again="getCheckPointDetail" @next-level="nextLevel"></water-box>
  </div>
</template>
<script>
  ...
  export default {
    ...
    created() {
      this.$nextTick(() => {
        this.$refs.endBg.addEventListener('canplay', () => {
          if (localStorage.getItem('isCloseBg') == 'false') {
            this.$refs.endBg.play();
          }
        });
      });
