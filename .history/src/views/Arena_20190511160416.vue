<template>
  <div class="chess-arena-wrapper">
    ...
    <chess-online-race-panel :fightLevel="fightLevel" :fightLogo="fightLogo" :fightName="fightName" @start-fight="startGame" :is-match-success="isMatchSuccess" :wait-time="waitTime"
            ...
            if (msg.data.indexOf("b login success") != -1) {
              ...
              if (!sessionStorage.getItem("user_type2")) {
                ...
                this.socket.send(
                  ...
                  type: "user",
                  content: "get-user"+JSON.stringify({
                    userId:this.userId
                  }),
                  user_type: sessionStorage.getItem("user_type")
                })}`
              ...
              switch (data.type) {
                case "user":
                  ...
                  else if(data.content.indexOf('get-user') != -1) {
                    alert(data.content.substring(8));
                    //用户发送userID给对方
                    this.getPersonInfo(JSON.parse(data.content.substring(8)).userId);
                  }
