<template>
  <div class="chess-online-race-wrapper">
    <!-- 背景音乐 -->
    <audio ref="onlineBg" preload="auto" muted loop>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$nextTick(() => {
        this.$refs.onlineBg.addEventListener('canplay', () => {
          if (localStorage.getItem('isCloseBg') == 'false') {
            this.$refs.onlineBg.play();
          }
        });
      });
      ...
      "isMove.value": {
        handler: function (a, b) {
          ...
          if (a == 18) {
            this.ImgShow = false;
            this.showCreateSucess = true;
            this.avter1 = require("../assets/images/不同意悔棋.png");
            setTimeout(() => {
              this.showCreateSucess = false;
            }, 2000);
            isMove.value = 1;
