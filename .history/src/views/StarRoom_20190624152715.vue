  ...
  <div class="chess-star-wrapper">
    ...
    <div class="inform">
      <div class="inform_left">
        <img :src="teacherInfo.picture == null ? require('../assets/images/user-logo.png') : teacherInfo.picture" alt>
      </div>
      ...
      <div class="swiper-container swiper-no-swiping">
        <div class="content-wrapper-container swiper-wrapper">
          <div class="swiper-slide" v-for="n in Math.ceil(teacherList.length / 7)" :key="n">
            <div @click="selectTeacher(teacher.id)" class="teacher-item pointer"
              :class="{active:currentId == teacher.id}" v-for="teacher in teacherList.slice(7*(n-1),7*(n-1)+7)"
              :key="teacher.id">
              <img :src="teacher.picture == null ? require('../assets/images/user-logo.png') : teacher.picture"
                class="teacher-logo">
    ...
    <!-- 教师详情 -->
    <chess-teacher-detail-panel @hide="closeTeacherDetail" :content="currentDetail" :is-show="showTeacherDetail">
    </chess-teacher-detail-panel>
  </div>
</template>
<script>
  ...
  export default {
    ...
    mounted() {
      this.$nextTick(() => {
        this.$refs.starBg.addEventListener('canplay', () => {
          if(localStorage.setItem('isCloseBg',this.isClose)) {
             this.$refs.starBg.play();
          }
        });
      });
      this.getTeacherList();
