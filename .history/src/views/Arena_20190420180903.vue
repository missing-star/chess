...
import {SearchEngine,countTimes,countTimes2,saveGameResult,preOperation} from '../assets/js/online/CChess'
export default {
    data() {
        return {
    ...
    methods:{
                        ...
                        switch (data.type) {
                            case 'user':
                                ...
                                if (data.content === 'yes' && !sessionStorage.getItem('user_type2')) {
                                    ...
                                    if (confirm('您同意对方悔棋吗')) {
                                        result = 'agree';
                                        moveOnline(preOperation.y, preOperation.x, preOperation.j, preOperation.i, preOperation.eat, true,true);
                                    } else {
                                        result = 'refuse';
                                    }
                                    this.socket.send(`${sessionStorage.getItem('uuid')}-${sessionStorage.getItem('user_type')}-${JSON.stringify({
        ...
        startGame() {
            ...
            //开始游戏，进入对战界面
            this.$router.push({name:'online-race',params:{isOnline:this.isOnline}});
        },
        getUuuid(len, radix) {
