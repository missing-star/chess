<template>
  <div class="chess-self-study-wrapper">
    <!-- 背景音乐 -->
    <audio ref="selfBg" preload="auto" muted loop>
      <source src="../assets/audio/自习室下棋的背景音乐.mp3">
    </audio>
...
<script>
  ...
  export default {
        ...
        this.$axios({
          ...
          .then(res => {
            if (res.data.status == 1) {
              ...
              this.answerList = JSON.parse(res.data.data.play_log);
            }
          })
          .catch(err => {
            console.log(err);
    ...
    mounted() {
      this.$nextTick(() => {
        this.$refs.selfBg.addEventListener('canplay', () => {
          if (localStorage.getItem('isCloseBg') == 'false') {
            this.$refs.selfBg.play();
          }
        });
      });
