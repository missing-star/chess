    ...
    methods:{
        ...
        goGame() {
            if (this.isOnline) {
                ...
                const uuid = `user${getUuuid(8, 16)}`;
                sessionStorage.setItem('uuid', uuid);
                this.goOnlineRace();
                // this.socket = new Webthis.socket('ws://47.99.241.87:1234');
                this.socket = new Webthis.socket('ws://127.0.0.1:8001');
                ...
                sessionStorage.setItem('isRed', true);
                sessionStorage.setItem('nowWho', 0);
                searchEngine = new SearchEngine(1);
                this.startGame(this.socket);
            }
        },
        startGame() {
            //开始游戏，进入对战界面
            this.$router.push({name:'online-race',params:{socket:this.socket,id:1}});
        },
        //计时一分钟
        countTimes(flag) {
            if (flag == 'over') {
