                        ...
                        <div class="swiper-container swiper-no-swiping">
                            <div class="content-wrapper-container swiper-wrapper">
                                <div v-for="n in Math.ceil(gradeList.length / 4)" :key="n" class="swiper-slide">
                                    <div class="grade-item pointer" :class="{active:(n-1)*4+index == currentGradeIndex}" @click="getStudentsByGradeId(item.id,$event)" v-for="(item,index) in gradeList.slice(4*(n-1),4*(n-1)+4)" :key="item.id">
                                    {{item.nickname}}
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        getStudentsByGradeId(id,e) {
            if(!e.target || Array.from(e.target.classList).includes('active')) {
                return false;
            }
            else {
