        ...
        <div class="swiper-container swiper-1 swiper-no-swiping">
          <div class="swiper-wrapper wrapper-1">
            <ul class="task-list-wrapper swiper-slide" v-for="n in Math.ceil(1/ 3)" :key="n">
              <li class="task-item" @click="selectLevel(0,0)">第一阶段</li>
              <li class="task-item" @click="selectLevel(0,1)">第二阶段</li>
              <li class="task-item" @click="selectLevel(0,2)">第三阶段</li>
            </ul>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    openCheckPointPanel(index) {
      //二级页面
      this.closeMyself();
      this.$emit("open-check-point-panel", this.currentIndex, index);
    },
    switchTab(index) {
