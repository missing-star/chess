                        ...
                        <div class="top-layer-wrapper">
                            <div class="login-wrapper">
                                ...
                                <template v-if="currentIndex == 0">
                                    <chess-input @trigger="triggerRole" readonly label="选择身份"
                                        type="text" width="100%" v-model="role"></chess-input>
                                    ...
                                    <div class="operation-group">
                                        ...
                                        <button @click="login" :disabled="!isCanLogin" :class="{active:isCanLogin}" class="login pointer"></button>
                                    </div>
                                </template>
                                <!-- 注册 -->
                                <template v-else-if="currentIndex == 1">
                                    <chess-input label="姓名" type="text" width="50%" v-model="username"></chess-input>
                                    <chess-input @trigger="triggerRole" readonly label="性别"
                                        type="text" width="100%" v-model="role"></chess-input>
                                    <chess-select :option-list="roleList" @select="selectRole" :is-show="showSelectRole"
                                        styles="position:absolute;right:15%;top:28%;z-index:1"></chess-select>
                                    <chess-input label="卡号" type="text" width="50%" v-model="password">
                                    <chess-input label="激活码" type="text" width="50%" v-model="password">
                                    <chess-input label="家长手机号" type="text" width="50%" v-model="password">
                                    <chess-input label="等级" type="text" width="50%" v-model="password">
                                        <chess-input label="学校" type="text" width="50%" v-model="password">
                                            <chess-input label="密码" type="password" width="50%" v-model="password">
                                    </chess-input>
                                    <div class="operation-group">
                                        <div class="password-operation">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            login() {
                if(this.rememberMe) {
                    localStorage.setItem('reInfo',JSON.stringify({username:this.username,pass:this.password}));
                }
                else {
