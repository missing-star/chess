    ...
    <div class="chess-dialog-body chess-growth-panel-body" :class="{'hide':!isShow}">
      ...
      <div class="content-wrapper-container">
        <div class="content left">
          ...
          <div class="race-result">{{growthLog.win_num}}胜{{growthLog.lose_num}}负{{growthLog.flat_num}}平</div>
        ...
        <div class="content right">
          ...
          <ul class="race-list-wrapper">
            <li v-for="(item,index) in growthLog.list" :key="index" class="race-item">
              ...
              <p class="race-result">
                <img :src="item.game==1? require('../assets/images/胜.png'):require('../assets/images/败.png')"
                  class="race-result-icon">
                <span class="race-title">{{item.game==1?"赢":(item.game==2?"平":"输")}}</span>
                <img @click="getRaceDetail(item.id)" src="../assets/images/右键头.png"
                  class="race-result-detail-icon pointer">
              </p>
            </li>
          </ul>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getRaceDetail(race_id) {
        this.$router.push(`/online-race-detail/${race_id}`);
      }
    },
