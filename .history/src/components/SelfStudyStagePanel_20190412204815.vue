      ...
      <div class="content-wrapper-container">
        <div class="category-wrapper">
          <div class="category_wrap" v-for="(item,index) in list" :key="index">
            <p
              @click="switchCase(index,item.id)"
              class="title pointer"
...
export default {
  ...
  methods: {
    ...
    getCategory(mainCatId,subCatId) {
      this.$axios({
        url:`${process.env.VUE_APP_URL}index.php?r=api-student/get-three-cate`,
        method:"post",
  ...
  created() {
    this.$axios({
      ...
      .then(res => {
        this.list = res.data.data[0].child[0].child;
      })
      .catch(error => {
        console.log(error);
      });
