            ...
            <div class="swiper-container swiper-no-swiping">
                <div class="content-wrapper-container swiper-wrapper">
                    <div class="swiper-slide" :class="{end:checkPointList.slice((n-1)*8,8*n).length > 4}" v-for="n in Math.ceil(checkPointList.length / 8)" :key="n">
                        <div @click="goCheckPoint(item.id,index)" :class="{six:index == 5}" class="teacher-item pointer" v-for="(item,index) in checkPointList.slice(8*(n-1),8*(n-1)+8)" :key="item.id">
                            ...
                            <p v-if="index != 0" class="show-guan-title">第{{index+1}}关</p>
                        </div>
                    </div>
                </div>
