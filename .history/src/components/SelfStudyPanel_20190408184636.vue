<template>
    <div class="chess-homework-panel chess-dialog-wrapper" :class="{'hide':!isShow}">
        ...
        <template v-if="currentIndex != 1">
            <!-- 平台公告遮罩点击层 -->
            <div class="tabbar-wrapper click-mask platform" @click="switchTab(1)" :class="{'hide':!isShow}"></div>
        </template>
        <template v-if="currentIndex != 2">
            <!-- 平台公告遮罩点击层 -->
            <div class="tabbar-wrapper click-mask other" @click="switchTab(2)" :class="{'hide':!isShow}"></div>
        </template>
        <div class="chess-dialog-body chess-homework-panel-body" :class="{'hide':!isShow,platform:currentIndex == 1,other:currentIndex == 2}">
            ...
            <div class="content-wrapper-container" :class="{active:currentIndex == 0}">
                <div class="pre-wrapper swiper-button-prev swiper-button-prev-1" id="prev-1">
                    <img disabled v-if="isFirstPage1" src="../assets/images/arrow-left-disabled.png" class="page-icon">
                ...
                <div class="next-wrapper swiper-button-next swiper-button-next-1" id="next-1">
                    <img disabled v-if="isLastPage1" src="../assets/images/arrow-right-disabled.png" class="page-icon">
                ...
                <div class="swiper-container swiper-1 swiper-no-swiping">
                    <div class="swiper-wrapper wrapper-1">
                        <ul class="task-list-wrapper swiper-slide" v-for="n in Math.ceil(taskList1.length / 8)" :key="n">
                            <li v-for="item in taskList1.slice(3*(n-1),3*(n-1)+3)" :key="item.id" class="task-item">
            ...
            <div class="content-wrapper-container" :class="{active:currentIndex == 1}">
                <div class="pre-wrapper swiper-button-prev swiper-button-prev-2" id="prev-2">
                    <img disabled v-if="isFirstPage2" src="../assets/images/arrow-left-disabled.png" class="page-icon">
                    <img v-else src="../assets/images/arrow-left-big.png" class="page-icon">
                </div>
                <div class="next-wrapper swiper-button-next swiper-button-next-2" id="next-2">
                    <img disabled v-if="isLastPage2" src="../assets/images/arrow-right-disabled.png" class="page-icon">
                    <img v-else src="../assets/images/arrow-right-big.png" class="page-icon">
                </div>
                <div class="swiper-container swiper-2 swiper-no-swiping">
                    <div class="swiper-wrapper wrapper-2">
                        <ul class="task-list-wrapper swiper-slide" v-for="n in Math.ceil(taskList2.length / 8)" :key="n">
                            <li v-for="item in taskList2.slice(3*(n-1),3*(n-1)+3)" :key="item.id" class="task-item">
            ...
            <div class="content-wrapper-container" :class="{active:currentIndex == 2}">
                ...
                <div class="swiper-container swiper-3 swiper-no-swiping">
                    <div class="swiper-wrapper wrapper-3">
                        <ul class="task-list-wrapper swiper-slide" v-for="n in Math.ceil(taskList3.length / 8)" :key="n">
                            <li v-for="item in taskList3.slice(3*(n-1),3*(n-1)+3)" :key="item.id" class="task-item">
...
export default {
    ...
    methods: {
        ...
        computedLength3() {
            return Math.ceil(this.taskList3.length / 8) > 1;
        }
    },
    mounted() {
        ...
        this.isLastPage3 = Math.ceil(this.taskList3.length / 8) > 1 ? false : true;
        this.$nextTick(() => {
            const vm = this;
            new Swiper('.swiper-1',{
                watchSlidesProgress:true,
...
<style scoped>
    ...
    li.task-item {
        ...
        position: relative;
        padding: 0.5rem;
        background: url(../assets/images/task-item.png) no-repeat;
    }
    .task-title-wrapper {
    ...
    .finish-status-wrapper {
        ...
        background-image:linear-gradient(-180deg, #ff8b32 0%, #ffaf5e 65%, #ffa34a 100%);
