...
function move(y, x, j, i, eat, isBack, isNext) {
    if (isBack) {
        ...
        if (vm.recordList[vm.currentIndex].targetElem == "" || isNext) {
            map[j][i] = map[y][x];
            map[y][x] = 0;
        } else {
            map[j][i] = map[y][x];
            map[y][x] = vm.recordList[vm.currentIndex].targetElem.value;
        }
        $("#CS" + j + "-" + i).html(
            "<section class='C " + vm.recordList[vm.currentIndex].sourceElem + "' style='transform:translate(" + (x - i) * 45 + "px," + (y - j) * 45 + "px);'>" + tex + "</section>"
        )
        //(未吃掉棋子和前进吃掉棋子的情况)
        if (vm.recordList[vm.currentIndex].targetElem.value == 0 || isNext) {
            $("#CS" + y + "-" + x).html(
                ""
            )
        } else {
            //后退
            $("#CS" + y + "-" + x).html(
                "<section class='C " + vm.recordList[vm.currentIndex].targetElem.cla + "'>" + tex + "</section>"
            )
        }
        vm.recordList[vm.currentIndex].flag = false;
        counts -= 1;
    } else {

        //通知走法记录是否需要切割
        var end = false;
        if (isBackOrGo || (vm.currentIndex != -1 && vm.currentIndex != vm.recordList.length - 1)) {
            //操作了前进/后退，手动移动棋子，删除当前记录后的操作记录
            vm.recordList = vm.recordList.slice(0, vm.currentIndex + 1);
            end = vm.currentIndex + 1;
        }
        //下棋操作
        onMove = true;
        if (eat == null)
        ...
        vm.recordList.push(preOperation);
        vm.currentIndex = vm.recordList.length - 1;
        counts += 1;
        showTarget(j, i, end);
    }
...
function showTarget(y, x, end) {
    if (end) {
        if (end % 2 == 0) {
            ...
            record = vm.showrecordList[parseInt(end / 2)];
        }
    }
    console.log(end, vm.showrecordList);
    if (source.t < 0) {
        //黑棋
        if (y - source.y != 0) {
            //进退
    ...
    if (vm.recordList.length != 0) {
        if (vm.recordList.length % 2 == 0) {
            vm.showrecordList[parseInt(vm.currentIndex / 2)] = record;
            record = {
                red: '',
                black: ''
            }
        } else {
            vm.showrecordList.push(record);
        }
    }
    source = {
        y: '',
 ...
 */
function backRecord() {
    if (vm.currentIndex == -1) return;
    preOperation = vm.vm.recordList[vm.currentIndex];
    move(preOperation.y, preOperation.x, preOperation.j, preOperation.i, preOperation.eat, true);
    vm.currentIndex -= 1;
}
 ...
 */
function nextRecord() {
    if (vm.currentIndex == vm.vm.recordList.length - 1) return;
    vm.currentIndex += 1;
    preOperation = vm.vm.recordList[vm.currentIndex];
