...
function showTarget(y, x, end) {
    var key = vm.isStu ? 'showRecordListStu' : 'showRecordList';
    if (end) {
        if (end % 2 == 0) {
            vm[key] = vm[key].slice(0, parseInt(end / 2));
        } else {
            vm[key] = vm[key].slice(0, parseInt(end / 2) + 1);
            vm[key][parseInt(end / 2)].black = '';
            vm.record = vm[key][parseInt(end / 2)];
        }
    }
    if (vm.source.t < 0) {
        //黑棋
    ...
    if (recordList.length != 0) {
        if (recordList.length % 2 == 0) {
            vm[key][parseInt(currentIndex / 2)] = vm.record;
            vm.record = {
                red: '',
                black: ''
            }
        } else {
            vm[key].push(vm.record);
        }
    }
    vm.source = {
        y: '',
