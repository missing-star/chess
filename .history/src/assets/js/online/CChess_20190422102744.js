...
//局时计时
function countRaceTime(flag) {
	if(flag == 'over') {
		clearInterval(intervalAll);
	}
	else {
		intervalAll = setInterval(() => {
			let times = new Date().getTime();
			let newDate = new Date(times - this.nowTimes);
			let hour = newDate.getHours()-8 < 10 ? '0'+(newDate.getHours()-8) : newDate.getHours();
			...
			showGameTime.value = hour +':'+ minute + ':' + second;
		}, 1000);
	}
}
function getEngineeMove(map) {
	...
	showSource(bestMove.from.y,bestMove.from.x,map[bestMove.from.y][bestMove.from.x]);
	setTimeout(function () {
...
/**
 * 保存比赛结果
 * @param{string} user_type --- 用户类型（a/b）
 * @param{string} socket_key --- 用户UUID
 */
function saveGameResult(user_type, socket_key) {
    $.ajax({
        url: `${api.url}/index.php?r=api-b/game-over-save-data`,
        type: 'post',
        dataType: 'json',
        async: false,
        data: {
...
function backOperation() {
    ...
    if (!isOnline.value) {
        setTimeout(function () {
            move(this.preOperation.y, this.preOperation.x, this.preOperation.j, this.preOperation.i, this.preOperation.eat, true,true);
            return;
        }, 1000);
    } else {
        //发送悔棋请求对方确认
