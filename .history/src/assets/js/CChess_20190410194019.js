...
function showTarget(y, x, end) {
    if (end) {
        if (end % 2 == 0) {
            ...
            vm.record = showRecordList[parseInt(end / 2)];
        }
    }
    if (source.t < 0) {
        //黑棋
        if (y - source.y != 0) {
            ...
            if ([4, 5, 6].indexOf(Math.abs(source.t)) != -1) {
                //马，相，士斜线走法的棋子
                vm.record.black = source.name + (x + 1 - source.x < 0 ? '退' : '进') + numToChara(Math.abs(x + 1));
            } else {
                vm.record.black = source.name + (y - source.y < 0 ? '退' : '进') + numToChara(Math.abs(y - source.y));
            }
        } else {
            //平
            vm.record.black = source.name + '平' + numToChara((x + 1));
        }
    } else {
        //红棋
        if (y - source.y != 0) {
            //进退
            if ([4, 5, 6].indexOf(Math.abs(source.t)) != -1) {
                vm.record.red = source.name + (source.x - (9 - x) > 0 ? '退' : '进') + numToChara(Math.abs(9 - x));
            } else {
                vm.record.red = source.name + (y - source.y > 0 ? '退' : '进') + numToChara(Math.abs(y - source.y));
            }
        } else {
            //平
            vm.record.red = source.name + '平' + numToChara((9 - x));
        }
    }
    if (recordList.length != 0) {
        if (recordList.length % 2 == 0) {
            showRecordList[parseInt(currentIndex / 2)] = vm.record;
            vm.record = {
                red: '',
                black: ''
            }
        } else {
            showRecordList.push(vm.record);
        }
    }
    source = {
        y: '',
