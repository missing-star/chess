...
function move(y, x, j, i, eat, isBack, isNext) {
    if (isBack) {
        ...
        if (vm.recordList[currentIndex].targetElem == "" || isNext) {
            map[j][i] = map[y][x];
            map[y][x] = 0;
        } else {
            map[j][i] = map[y][x];
            map[y][x] = vm.recordList[currentIndex].targetElem.value;
        }
        $("#CS" + j + "-" + i).html(
            "<section class='C " + vm.recordList[currentIndex].sourceElem + "' style='transform:translate(" + (x - i) * 45 + "px," + (y - j) * 45 + "px);'>" + tex + "</section>"
        )
        //(未吃掉棋子和前进吃掉棋子的情况)
        if (vm.recordList[currentIndex].targetElem.value == 0 || isNext) {
            $("#CS" + y + "-" + x).html(
                ""
            )
        } else {
            //后退
            $("#CS" + y + "-" + x).html(
                "<section class='C " + vm.recordList[currentIndex].targetElem.cla + "'>" + tex + "</section>"
            )
        }
        vm.recordList[currentIndex].flag = false;
        counts -= 1;
    } else {

        //通知走法记录是否需要切割
        var end = false;
        if (isBackOrGo || (currentIndex != -1 && currentIndex != vm.recordList.length - 1)) {
            //操作了前进/后退，手动移动棋子，删除当前记录后的操作记录
            vm.recordList = vm.recordList.slice(0, currentIndex + 1);
            end = currentIndex + 1;
        }
        //下棋操作
        onMove = true;
        ...
        currentIndex = vm.recordList.length - 1;
        counts += 1;
        showTarget(j, i, end);
    }
...
function showTarget(y, x, end) {
    ...
    if (vm.recordList.length != 0) {
        if (vm.recordList.length % 2 == 0) {
            vm.showrecordList[parseInt(currentIndex / 2)] = record;
            record = {
                red: '',
                black: ''
            }
...
function backRecord() {
    ...
    preOperation = vm.vm.recordList[currentIndex];
    move(preOperation.y, preOperation.x, preOperation.j, preOperation.i, preOperation.eat, true);
    currentIndex -= 1;
}
 ...
 */
function nextRecord() {
    if (currentIndex == vm.vm.recordList.length - 1) return;
    currentIndex += 1;
    preOperation = vm.vm.recordList[currentIndex];
