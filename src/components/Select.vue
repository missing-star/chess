<template>
    <ul class="chess-select-wrapper" :style="styles" v-show="isShow">
        <li :key="item.id" v-for="item in optionList" :class="{'select-item-wrapper':true,'active':currentId == item.id}" @click="selectOption(item.id)">
...
<style scoped>
    ...
    li.select-item-wrapper.active img.select-icon.active,li.select-item-wrapper img.select-icon.not-active{
        display: block;
    }
    li.select-item-wrapper.active img.select-icon.not-active,li.select-item-wrapper img.select-icon.active{
        display: none;
    }
</style>
